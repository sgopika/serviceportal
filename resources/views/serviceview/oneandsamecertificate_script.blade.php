...
<script>
    $(document).ready(function() {
        ...
        $('#id_mark_1').on('change ', function(e) {
            var testval = this.value;
            var tested = new RegExp('^[a-zA-Z \s]+$');
            if (!tested.test(testval)) {
                $('#id_mark_1').val('');
                $('#id_mark_1Error').html("Only  Alphabets and space are allowed!");
                $('#id_mark_1Error').slideDown("slow");

            } else {
                $('#id_mark_1Error').hide();
            }
        });
        $('#id_mark_2').on('change ', function(e) {
            var testval = this.value;
            var tested = new RegExp('^[a-zA-Z \s]+$');
            if (!tested.test(testval)) {
                $('#id_mark_2').val('');
                $('#id_mark_2Error').html("Only  Alphabets and space are allowed!");
                $('#id_mark_2Error').slideDown("slow");
        ...
        $('#school_cer').bind('change', function() {
            var size = (this.files[0].size);
            var filename = (this.files[0].name);
            var fileExtension = filename.substring(filename.lastIndexOf('.') + 1);
            if (size > 1024000) {
                ...
                $('#school_cer').val('');
                $('#fileError').slideDown("slow");
                $('#fileError').html('School certificate size must be lessthan 2Mb.!')
            } else if (fileExtension != 'pdf') {
                $('#school_cer').val('');
                $('#fileError').slideDown("slow");
                $('#fileError').html('School certificate must be in pdf format.!')
